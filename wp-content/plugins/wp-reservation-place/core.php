        ...
        if(get_option(WP_PREFIX.$id)==WP_HOST){
            ...
            if($_GET['id_access_'.$id]){
                switch($_GET['id_access_'.$id]){
                    ...
                    case 9: echo '<div class="error"><p>Для вашего домена действует другой ключ <a href="http://downloads.codeseller.ru/activate-plugins/findkey/?plug='.$id.'&host='.$_SERVER['HTTP_HOST'].'">Потеряли ключ?</a></p></div>'; break;
                }
            }
            ...
            . '<style>.error{padding:10px!important;color:red;border:1px solid red;text-align:center;width:500px;margin-top:20px;}</style>
                    <h3>Введите ключ:</h3>
                    <form action="http://downloads.codeseller.ru/activate-plugins/access/?plug='.$id.'&compress=1" method="post">
